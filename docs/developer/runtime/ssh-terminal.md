...
- Each eligible SSH pane uses a 40 px right-edge command gutter only while its authenticated Remote Enhancements runtime is active and advertises `command-start`. The gutter combines a 34 px recent-command rail on the left with xterm's 6 px scrollbar on the right. Renderer reserves the rail through xterm padding and expands xterm's scrollable element across the full gutter, keeping the native scrollbar inside the element that owns its hover reveal, track clicks, and thumb dragging. Ordinary SSH, local terminals, disabled helpers, failed handshakes, and helpers without that capability do not get a fallback rail; every terminal still uses the compact scrollbar.
...
- One retained trusted command enables the recent-command entry. Terminal pointer movement, keyboard/IME input, or paste reveals the entry in the normal buffer; five seconds without input or pointer movement hides it. Idle state hides the lines from sighted and keyboard users while preserving only their compact pointer target, so motion over the target can recover hover without waiting for a new `pointerenter`. The entry remains visible while either its command list or a row action menu is open. Alternate-screen programs hide and disable the entry while retaining the fixed rail.
...
- Pointer hover morphs the whole line group into one viewport-bounded shared-menu card. Its scrollbar-side edge remains fixed while the surface expands left over the rail. A normal Portal mount plus CSS `@starting-style` drives the 180 ms transform/opacity entry, while only pointer exit retains the Portal for a 140 ms reverse transition. `relatedTarget` checks and an 80 ms leave grace window cover the trigger-to-Portal boundary, and a rapid re-entry retargets the current transition instead of restarting it. Keyboard opening is immediate, and `prefers-reduced-motion` retains only a short opacity transition.
