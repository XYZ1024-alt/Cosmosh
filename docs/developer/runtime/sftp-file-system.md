...
- Public task snapshots exist only in backend memory, remain queryable after a recent session close, and are retained for up to seven days after runner release. Each session retains at most 512 records; pressure evicts the oldest released terminal snapshots before rejecting more work, and backend shutdown clears all remaining records. Renderer task state independently owns the shorter user-attention lifecycle. The task collection has no public cancel or resume route and no persistence; archive-specific cancellation remains a separate archive API.
