...
- Previous/next command actions do not belong in the terminal context menu. Once authenticated Remote Enhancements are active with `command-start`, each eligible pane reserves a fixed 40 px rail on its right edge; all click and navigation actions carry that pane's explicit id.
- Show timeline content only after the pane retains at least three trusted commands and the normal xterm buffer has scrollback (`baseY > 0`). Keep the reserved rail empty below that threshold and during alternate-screen programs so content visibility never changes xterm/PTY columns.
- Timeline markers appear in submission order inside a vertically centered adaptive group. Its height is `min(availableHeight, commandCount * 16px)`, so it expands equally upward and downward before compressing spacing at the available-height limit; the newest command occupies the bottom row.
- Marker widths use `clamp(8 + 4 * log2(outputRows + 1), 8, 28)` CSS pixels and the semantic `color.ssh.terminal.command-timeline.*` token family. Hover exposes the complete command, click reveals its input row, and top/bottom chevrons navigate without wrapping. At the bottom of scrollback the newest command uses the active token; after scrolling upward, the command at the viewport center becomes active.
