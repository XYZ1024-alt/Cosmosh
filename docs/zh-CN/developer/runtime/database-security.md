...
- 生产模式强依赖支持 SQLCipher 的 native driver。若 `better-sqlite3-multiple-ciphers` 无法加载，Backend 会以 `DB_SQLCIPHER_BOOTSTRAP_FAILED` 失败，而不会通过明文 Prisma SQLite 兼容模式打开数据库。
