...
- SFTP 设置控制重连模式、删除确认的触发范围、中间文件列表是否显示开头的 `..` 父目录行，以及地址栏是否始终以文本形式显示。
...
- **Settings registry**：`packages/api-contract/src/settings-registry.ts` 负责 renderer settings store 消费的 SFTP 重连、删除确认、父目录行、隐藏条目与地址显示偏好。
