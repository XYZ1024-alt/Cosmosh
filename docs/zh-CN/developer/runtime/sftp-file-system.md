...
- SFTP 设置控制重连模式、删除确认的触发范围、文件列表列/排序视图状态、中间文件列表是否显示开头的 `..` 父目录行，以及地址栏是否始终以文本形式显示。
- Backend 写操作支持空文件创建、目录创建、重命名/移动、递归复制与递归删除。

v1 明确不包含：

- 目录下载、chmod、拖放、全局搜索、完整应用内文件编辑，以及带取消/冲突处理的 backend 级传输队列。
...
- **Settings registry**：`packages/api-contract/src/settings-registry.ts` 负责 renderer settings store 消费的 SFTP 重连、删除确认、目录列表视图、父目录行、隐藏条目与地址显示偏好。
...
| `POST` | `/api/v1/sftp/sessions/{sessionId}/upload` | 冲突检查通过后，将一个本地已编辑的 SFTP 临时文件流式写回原远程文件；`overwrite: true` 只在 renderer 冲突确认后使用。 |
