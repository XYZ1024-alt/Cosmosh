...
- 目录上传/下载、chmod、外部或跨标签页拖放目标、空白区域/文件行/文本地址栏拖放目标、全局搜索，以及带字节级进度与取消的 backend 级传输队列。
...
- 内部 SFTP 拖拽遵循选择归属：拖拽未选中的行时只拖动该行；拖拽已选中行时拖动当前多选集合。首版只接受同一标签页内的明确目录 drop target：目录树行、中间列表目录行、面包屑目录段和地址栏目录下拉项。拖到空白区域、文件行、地址文本输入、外部文件、另一个 SFTP 标签页，或拖到已选目录自身/后代目录时，会对整组拖拽刻意忽略。
...
4. 在安全边界与目标解析规则定义清楚后，补充外部文件、跨标签页、空白区域与文件行拖放目标。
