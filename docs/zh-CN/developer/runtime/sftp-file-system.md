...
- Renderer 展示目录项与元数据详情。双击普通文件会将其下载到 Cosmosh 受控的 SFTP 临时目录，并使用系统默认应用打开。
- 左侧目录树展示当前目录的父级链路，并在用户浏览时缓存已加载的子目录，还提供目录作用域的右键操作：打开、在新标签页打开、刷新、粘贴、新建文件与新建文件夹。
- 中间列表右键菜单与顶部操作栏提供打开、文件夹新标签打开、在此处打开 SSH、复制地址、复制相对地址、保存普通文件到本地、支持平台上的打开方式、剪切、复制、粘贴、删除、新建文件、新建文件夹与行内重命名。当存在排队、运行中或刚完成的 SFTP 操作时，顶部操作栏会显示任务下拉。目录列表支持 `Ctrl`/`Cmd` 切换多选与 `Shift` 范围选择。
- SFTP 设置控制删除确认的触发范围，以及中间文件列表是否显示开头的 `..` 父目录行。
- Backend 写操作支持空文件创建、目录创建、重命名/移动、递归复制与递归删除。

v1 明确不包含：

- 上传、目录下载、chmod、拖放、全局搜索、文件编辑，以及带取消能力的持久传输队列。
...
- **Renderer**：`packages/renderer/src/pages/SFTP.tsx` 负责标签页作用域 UI 状态、文件操作、renderer 任务队列、行内重命名/新建状态与预览状态。
...
Renderer 当前显示名称、大小、修改时间与 mode 列。目录面板只支持过滤当前目录条目，不是远端递归搜索。详情面板在单选时展示已选条目的元数据，多选时展示已选择数量。启用 `sftpShowParentDirectoryEntry` 且 backend 返回父路径时，中间列表会在真实条目前添加一个不可选择的 `..` 行，用于返回上一级目录且不改变 backend 数据。
...
- 用户触发的文件操作由 renderer 入队，而不是冻结整个 SFTP 工作台。任务运行时该标签页仍可继续接受其他操作；队列中的任务会针对同一个 backend SFTP 会话顺序执行，避免共享远端子系统上的操作竞态。
...
- 多条目剪切、复制、删除与粘贴会对当前 SFTP 会话发起一次 backend 批量 API 请求。Service 按顺序执行条目，遇到第一个失败后停止，返回每个条目的 `success`/`failed`/`skipped` 结果，且不会回滚已经完成的条目。重命名、打开、打开方式与新标签打开仍是单条目动作。
...
- 操作成功后会使当前可见目录缓存失效，并在后台重新校验列表；在服务器结果返回前保留当前列表、过滤条件与选择状态。
...
- 顶部工具栏保持紧凑，并按路径控制、远程路径输入、临时任务下拉、文件操作按钮与当前目录过滤的顺序排列。
- 工具栏分割线使用 `MenubarSeparator`，确保分割线尺寸与颜色跟随共享菜单 token。
- 仅在有任务排队、运行中或处于短暂完成保留窗口时渲染任务控制。触发器使用图标、本地化标签、紧凑数量徽标、tooltip，以及共享 `DropdownMenu` surface，并通过自定义内容 slot 渲染任务行和进度条。
