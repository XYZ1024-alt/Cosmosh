...
- 右键菜单与顶部操作栏提供打开、文件夹新标签打开、剪切、复制、粘贴、删除、新建文件、新建文件夹与行内重命名。目录列表支持 `Ctrl`/`Cmd` 切换多选与 `Shift` 范围选择。
...
Renderer 当前显示名称、大小、修改时间与 mode 列。目录面板只支持过滤当前目录条目，不是远端递归搜索。详情面板在单选时展示已选条目的元数据，多选时展示已选择数量，并在打开普通文件后切换为有上限的预览。
...
- 多条目剪切、复制、删除与粘贴由 renderer 按顺序编排为当前 SFTP 会话上的单条目 API 调用。重命名、打开、新标签打开与预览仍是单条目动作。
...
- 避免工具栏 overflow 菜单与右键菜单之间出现重复项。行右键菜单聚焦已选条目，空白区域右键菜单聚焦粘贴/新建动作，工具栏 overflow 菜单只放没有独立工具栏按钮的动作。
